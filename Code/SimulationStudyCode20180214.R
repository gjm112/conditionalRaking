...
pop1$income <- 25000 + 
  ...
  40000 * pop1$I_age_old * pop1$I_race_B * pop1$I_ins_A + 
...
# Trial 4 Completely Stratified w/ Diff Pop -----------------------------------------------------------------

#### 4: completely stratified samp (using sample_n), w/ different population balance

pop4 <- data.frame(I_age_old = rbinom(N, 1, 0.8), I_sex_F = 0, I_race_B = 0, I_ins_A = 0)
#sex
...
pop4$income <- 25000 + 
  ...
  40000 * pop4$I_age_old * pop4$I_race_B * pop4$I_ins_A + 
...
p4 <- ggplot(res4) +
  ...
  geom_histogram(aes(x = value, fill = key), bins=100) +
  ggtitle("Completely Strat, less balanced pop") +
  xlim(60000, 100000) +
  ylim(0, 100)
p4
...
# Trial 5 -----------------------------------------------------------------
################ 5: completely stratified with a different unbalanced pop
pop5 <- data.frame(I_age_old = rbinom(N, 1, 0.8), I_sex_F = 0, I_race_B = 0, I_ins_A = 0)
#sex
p <- expit(1 - 0.9 * pop5$I_age_old)
pop5$I_sex_F <- rbinom(N,1,prob = p)
#race
p <- expit(0 + 0.9 * pop5$I_age_old * pop5$I_sex_F)
pop5$I_race_B <- rbinom(N,1,prob = p)
#Insurance
p <- expit(0 + pop5$I_race_B + 
             1.8 * pop5$I_sex_F * pop5$I_race_B + 
             1.2 * pop5$I_age_old * pop5$I_race_B)
...
pop5$income <- 25000 + 
  ...
  40000 * pop5$I_age_old * pop5$I_race_B * pop5$I_ins_A + 
...
p5 <- ggplot(res5) +
  ...
  geom_histogram(aes(x = value, fill = key), bins=100) +
  ggtitle("Completely Strat, different pop") +
  xlim(60000, 100000) +
  ylim(0, 100)
p5
